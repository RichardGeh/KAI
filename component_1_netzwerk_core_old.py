...
def sanitize_for_logging(text: str) -> str:
    """
    Entfernt cp1252-inkompatible Unicode-Zeichen für Windows-Logging.

    FIX: Implementiert für Windows cp1252 Kompatibilität (Code Review 2025-11-21, Concern 5)

    Args:
        text: Input-Text mit potentiell problematischen Unicode-Zeichen

    Returns:
        Sanitierter Text der sicher in cp1252 geloggt werden kann
    """
    if not text:
        return text

    try:
        # Versuche Encoding/Decoding mit 'replace' für inkompatible Zeichen
...
class KonzeptNetzwerkCore:
    ...
    def close(self):
        """
        Schließt die Neo4j-Verbindung und räumt Caches auf.

        FIX: Cache-Cleanup hinzugefügt (Code Review 2025-11-21, Concern 4)
        """
        if self.driver:
            self.driver.close()

        # Cleanup Caches um Memory Leaks zu vermeiden
        self._fact_cache.clear()
        self._known_words_cache.clear()
        logger.debug("Neo4j-Verbindung geschlossen und Caches geleert")
    ...
    def health_check(self) -> bool:
        """
        Prüft ob die Neo4j-Verbindung funktioniert.

        FIX: Hinzugefügt für Monitoring und Production-Readiness (Code Review 2025-11-21, Suggestion 6)
    ...
    def set_wort_attribut(
        ...
        Returns:
            True wenn erfolgreich gesetzt, False bei Fehler
        """
        if not self.driver:
        ...
        with self.driver.session(database="neo4j") as session:
            # SCHRITT 1: Setze Attribut mit RETURN für Verifikation
            query = f"""
            ...
            # Vergleiche gesetzten Wert (bei komplexen Typen kann Vergleich schwierig sein)
        ...
        # FIX: Cache-Invalidierung nach Wort-Modifikation (Code Review 2025-11-21, Concern 2)
        cache_key = f"known:{lemma_lower}"
        if cache_key in self._known_words_cache:
            del self._known_words_cache[cache_key]
            logger.debug(f"Known-Words-Cache invalidiert für '{lemma_lower}'")
    ...
    def _add_bedeutung(self, lemma: str, bedeutung_text: str) -> Dict[str, bool]:
        ...
        # FIX: Cache-Invalidierung nach Relation-Hinzufügung (Code Review 2025-11-21, Concern 2)
        lemma_lower = lemma.lower()
        cache_key = f"facts:{lemma_lower}"
        if cache_key in self._fact_cache:
            del self._fact_cache[cache_key]
            logger.debug(f"Fact-Cache invalidiert für '{lemma_lower}'")
    ...
    def _add_synonym(self, lemma1: str, lemma2: str) -> Dict[str, bool]:
        ...
        # FIX: Cache-Invalidierung nach Synonym-Hinzufügung (Code Review 2025-11-21, Concern 2)
